defmodule YachanakuyWeb.Staff.MaterialDeliveryLive do
  ...
  def handle_event("scan_qr", %{"qr_code" => qr_code}, socket) do
    ...
    if attendee do
      # Verificar si ya se entregaron los materiales
      if attendee.material_entregado do
        socket = assign(socket, 
          error_message: "Los materiales ya fueron entregados previamente a este participante."
        )
        {:noreply, socket}
      else
        # Actualizar el estado de entrega de materiales
        updated_attendee = update_material_delivery(attendee, socket.assigns.current_user)
        
        socket = assign(socket, 
          attendee: updated_attendee,
          scanned_qr: qr_code,
          success_message: "Materiales registrados como entregados exitosamente"
        )
        {:noreply, socket}
      end
    else
  ...
  defp update_material_delivery(attendee, user) do
    # Esta función simula la actualización del estado de entrega de materiales
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      <h1 class="text-3xl font-bold mb-8 text-[#144D85]">Escanear Materiales</h1>
      ...
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <%= if @attendee do %>
          <div class="border border-gray-200 rounded-lg p-6 mb-4">
            ...
            <div class="mt-4">
              <p class="font-semibold">Estado de Materiales:</p>
              <span class={
                ...
                <%= if @attendee.material_entregado, do: "Entregados", else: "Pendientes" %>
              </span>
            </div>
            
            <%= if !@attendee.material_entregado && @scanned_qr do %>
