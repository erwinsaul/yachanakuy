defmodule YachanakuyWeb.UserAuth do
  use YachanakuyWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias Yachanakuy.Accounts
  alias Yachanakuy.Accounts.Scope

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in UserToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_yachanakuy_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      YachanakuyWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  @doc """
  Used for LiveView routes that need current_scope assigned.
  """
  ...
  Used in admin live_session routes.
  """
  ...
  @doc """
  Ensures the user has staff role (operador or admin).
  Used in staff live_session routes.
  """
  def on_mount(:ensure_staff, _params, session, socket) do
    ...
    if socket.assigns[:current_scope] && 
      ...
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "No tienes permisos de staff.")
        |> Phoenix.LiveView.redirect(to: ~p"/")
  ...
  @doc """
  Ensures the user has supervisor role (encargado_comision or admin).
  Used in supervisor live_session routes.
  """
  def on_mount(:ensure_supervisor, _params, session, socket) do
    socket = mount_current_user(socket, session)
    ...
    if socket.assigns[:current_scope] && 
      ...
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "No tienes permisos de encargado.")
        |> Phoenix.LiveView.redirect(to: ~p"/")
